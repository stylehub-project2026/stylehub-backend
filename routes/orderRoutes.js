...
const { createOrder, getMyOrders, getOrder, cancelOrder, initiatePaymobPayment } = require('../controllers/orderController');
const { protect, customerOnly } = require('../middleware/authMiddleware');

router.post('/', protect, customerOnly, createOrder);
router.post('/paymob', protect, customerOnly, initiatePaymobPayment);
